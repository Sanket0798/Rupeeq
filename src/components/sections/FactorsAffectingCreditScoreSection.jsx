const FactorsAffectingCreditScoreSection = () => {
  ...
  return (
    <div className="w-full py-16 md:py-20 px-4 md:px-6 lg:px-8">
      <div className="max-w-[1386px] mx-auto rounded-24 z-10 bg-gradient-to-b from-[#34CA8D]/5 to-white pt-[55px] px-[57px] pb-[83px]">
        <div className="flex flex-row gap-x-[67px] items-center">
          {/* Left Side - Heading */}
          <div className="flex flex-col items-start">
            <h2 className="font-bold text-[40px] leading-[48px] tracing-[0px] text-[#212121] mb-11">
              Factors That <br />
              <span className="text-custom-dark-blue">Affect Your Credit Score</span>
            </h2>
            <div className="text-[#4B5768]">
              <h3 className="text-[30px] leading-[130%] font-semibold mb-6">
                We're determined to help you achieve your financial goals.
              </h3>
              <p className="text-xl font-normal leading-[130%] max-w-[392px] w-full">
          ...
          {/* <div className=""> */}
          <div className="grid grid-cols-2 max-w-[703px] gap-y-[35px] gap-x-[17px]">
            {factors.map((factor, index) => (
              <div
                key={index}
                className={`${factor.bgColor} ${factor.borderColor} border border-black/10 border-b-transparent rounded-t-3xl p-6 text-center transition-all duration-300 hover:scale-105 hover:shadow-lg flex items-center justify-center h-[119px]`}
              >
                <p className="text-custom-dark-text text-[25px] leading-tight font-light">
                  {factor.text}
                </p>
              </div>
            ))}
